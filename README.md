...
**最简单的方式**：双击项目根目录的 [一键启动.bat](./一键启动.bat)，它会自动检查依赖、构建界面并启动应用；音色服务在后台静默运行（日志在 `voice-clone/server.log`），**启动器完成后自动关闭**——全程只有一个应用窗口。
...
- **长期记忆**：每条用户消息后，后台整理「关于你的重要信息」存入 `memory.json`，下次对话注入；记忆按类型分类（身份/喜好/日期/关系/状态），身份信息「同一项只保留最新一条」，避免自相矛盾；记忆有上限，成本可控。
