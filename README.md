...
- **活人感 Alive-feeling interactions** — 打字机逐字输出、随机思考延迟、安静时主动找话题、时间段问候。Typewriter output, natural thinking delays, proactive messages, time-aware greetings.
- **语音朗读 Text-to-speech** — 神经语音朗读，文字与语音同步呈现；自动跳过（动作描写）等舞台说明。Neural voice reading with text-voice sync; stage directions are filtered before speaking.
...
| 对话模型 Dialogue model | OpenAI 兼容 Chat Completions API |
...
**一键启动 One-click launch**：双击 [一键启动.bat](./一键启动.bat)——自动检查依赖、构建界面、拉起后台音色服务（若已配置）并启动应用，启动器完成后自动关闭，全程只有一个窗口。Double-click it and everything runs: dependencies, build, background voice server, app — only one window remains.
...
- **自动朗读 Auto-read**：默认开启，文字与语音同步呈现。Enabled by default with text-speech sync.
...
- 托盘图标右键可管理：显示 / 隐藏主窗口、悬浮球开关、开机自启、退出。Right-click the tray icon to show/hide, toggle the floating ball, enable auto-launch, or quit.
...
├── electron/
│   ├── main.js                   # 主进程：窗口、托盘、模型调用、存储 Main process
│   └── preload.cjs               # 预加载：安全暴露 window.api Preload
├── src/
│   ├── App.jsx                   # 应用主组件 Main component
│   ├── styles.css
│   ├── shared/                   # 角色卡、记忆、语音过滤 Shared logic
...
- 聊天记录与长期记忆保存在应用数据目录（`%APPDATA%\神里绫华 AI 陪伴\`），纯本地存储。Chat history and memory live in the app data directory — local only.
